...
struct ServerContext {
  ...
  void Server(int num) {
    ...
    if (config.gid_idx >= 0) {
      int rc = ibv_query_gid(dev_info.ib_ctx, config.ib_port, config.gid_idx,
                             &my_gid);
      if (rc) {
        cerr << "无法获取本地GID" << endl;
        exit(0);
    ...
    if (sock_sync_data(connfd, sizeof(RdmaUDConnExchangeInfo),
                       (char *)&local_info, (char *)&tmp_info) < 0) {
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < cq_size; i++)
    post_recv(s_ctx.buf + i * (PACKET_SIZE + 40) % BUF_SIZE, PACKET_SIZE + 40,
              s_ctx.mr->lkey, s_ctx.qp, 1);
  ...
  while (1) {
    ...
    if (recv_wait < cq_size)
      post_recv(s_ctx.buf + pos, PACKET_SIZE + 40, s_ctx.mr->lkey, s_ctx.qp, 1),
          pos = (pos + PACKET_SIZE + 40) % BUF_SIZE, recv_wait++;
